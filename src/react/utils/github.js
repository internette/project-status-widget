export const getAdditionalPrDetails = async ({
  octokit,
  prNumber,
  repository
}) => {
...
export const getPrNotifications = async ({ octokit, prNumber, repository }) => {
  const allNotifications = await octokit
    .paginate(octokit.rest.activity.listNotificationsForAuthenticatedUser)
    .then((notifications) => notifications);
  const notificationsForPr = allNotifications.filter((notification) => {
    const matchesPr =
      notification.subject.url.indexOf(
        `${repository.owner}/${repository.name}/pulls/${prNumber}`
      ) >= 0;
...
export const getPrs = async ({ username, octokit, isUpdate = false }) => {
  ...
  const response = await octokit.request(fullSearch, {
    headers: {
      "X-GitHub-Api-Version": "2022-11-28"
    }
  });
  ...
  const prsWithRepoInfo = Promise.all(
    prs.map(async (pr) => {
      ...
      const repoName = repo.substring(repo.lastIndexOf("/") + 1, repo.length);
      ...
      if (isUpdate) {
        const notifications = await getPrNotifications({
          octokit,
          prNumber: pr.number,
      ...
      const additionalPrDetails = await getAdditionalPrDetails({
        octokit,
        prNumber: pr.number,
        repository: prDetails.repository
      });
